...
import { BookmarkDocument } from './models/bookmark.shcema';
...
export class BookmarksService {
    ...
    async getBookmarks(userId: string) {
        const bookmarkDocument = await this.bookmarksRepository.find({ userId });
        return bookmarkDocument.map((bookmark) => this.toModel(bookmark));
    }

    async getBookmark(
        getBookmarkArgs: GetBookmarkArgs,
        userId: string,
    ) {
